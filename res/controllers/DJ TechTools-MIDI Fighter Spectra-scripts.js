            ...
            for (let i = 0; i < 4; i++) {
                this.stemLayer[i] = new components.Button({
                    ...
                    connect: function() {
                        ...
                        console.log(`--- LOGGER: ${channel}`);
                        this.connections[2] = engine.makeConnection(this.group, "stem_count", this.stemCountChanged.bind(this));
                        console.log(`--- CONN: ${this.connections[2]}`);
                    },
                    stemCountChanged: function(value) {
                        console.log(`--- HERE: ${stemCount}, ${this.stemNum}`);
                        if (this.stemNum < value) {
                            this.send(this.off);
                        }
                    },
                    output: function(value, _group, control) {
                        ...
                        console.log(`--- LOGGER2: ${channel}, ${stemCount}`);
                        ...
                        // If muted, use the bright variant of the color,
                        // otherwise use the dim version.
                        this.send(nearestColorValue - (muted ? 1 : 0));
                    },
                });
            }
